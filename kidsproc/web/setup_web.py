...
# Package meta-data.
NAME = "monitor"
DESCRIPTION = "A real-time monitor of KIDS data."
...
if not VERSION:
    with open(
        os.path.join(here, "src", "monitor", "__version__.py")
    ) as f:
        exec(f.read(), about)
else:
...
setup(
    ...
    scripts=["bin/run-monitor-prod"],
    entry_points={
        "console_scripts": [
            "run-monitor-dev=monitor.dev_cli:main"
        ]
    },
)
