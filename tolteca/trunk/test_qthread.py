...
class Test(QtCore.QObject):
    ...
    _stopped_event = False

    @QtCore.pyqtSlot()
    def start(self):
        self._stop_loop = True
        while not self._stopped_event:
            time.sleep(1)
...
class firstGui(QtWidgets.QDialog):
    ...
    def init_ui(self):
        ...
        for j in range(self.n):
            self.starts[j].clicked.connect(self.threads[j].start)
