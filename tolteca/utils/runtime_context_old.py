...
class RuntimeContext(DirConfMixin):
    ...
    _setup_info_key = 'setup'
    _runtime_info_key = 'runtime'
    ...
    def __init__(self, rootpath=None, config=None):
        ...
        # in property ``config``
        self._config = config
        if not self.is_persistent and self.setup_info is None:
    ...
    def rootpath(self):
        ...
        return self.runtime_info['rootpath']
    ...
    @cached_property
    def config(self):
        """The runtime context config dict.

        """
        # while we collect the config, we'll update the runtime info
        # accordingly
        if self.is_persistent:
            ...
            # add dir content paths to the runtime info
            # note that this will overwrite existing entries
            self._update_runtime_info(cfg, self.to_dict())
        else:
            cfg = self.validate_config(self._config)
            # in this case there is no dir content. but we'll nevertheless
            # add the relevant keys if they are *not* present.
            for attr in self._get_to_dict_attrs():
                cfg[self._runtime_info_key].setdefault(attr, None)
        # This breaks the test wen setup is not available
        # add some more info about the context state
        # self._update_runtime_info(cfg, {
        #     'is_persistent': self.is_persistent,
        #     'config_has_setup': self._setup_info_key in cfg
        #     })
        self.logger.debug(f"loaded config: {pformat_yaml(cfg)}")
        return cfg

    @classmethod
    def _update_runtime_info(cls, cfg, runtime_info):
        # update the runtime info dict with `runtime_info` in `cfg`
        rupdate(cfg, {cls._runtime_info_key: runtime_info})

    @classmethod
    def _update_setup_info(cls, cfg, setup_info):
        # update the setup info dict with `setup_info` in `cfg`
        rupdate(cfg, {cls._setup_info_key: setup_info})
    ...
    def extend_config_schema(cls):
        ...
        return {
            ...
            Optional('runtime', default=dict): dict,
            Optional(str): object
            }
    ...
    def from_dir(
        ...
        **kwargs : dict, optional
        ...
        if kwargs.get('dry_run', False):
            ...
            cfg = dict()
            cls._update_runtime_info(cfg, {'rootpath': dirpath})
    ...
    def runtime_info(self):
        """The runtime dict in config."""
        # the runtime key should always be in the dict because we add
        # it in the config property getter.
        return self.config[self._runtime_info_key]

    @property
    def setup_info(self):
        """The setup dict in config."""
        return self.config.get(self._setup_info_key, None)
    ...
    def setup(self, config=None, overwrite=False):
        ...
        if setup_cfg is None:
            setup_cfg = dict()
        if self._setup_info_key in setup_cfg:
            if overwrite:
                self.logger.debug(
