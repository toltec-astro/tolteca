...
class ToltecObsSimulatorConfig(DataclassNamespace):
    ...
    _namespace_to_dict_schema = Schema({"hwp": Use(lambda c: c.to_dict()), str: object})
    ...
    def __init__(self, **kwargs):
        ...
        calobj = self.calobj = _load_calobj(self.calobj_index, allow_fallback=True)
        ...
        self.simulator = simu_cls(
            array_prop_table=apt, polarized=self.polarized, hwp_config=self.hwp
        )
...
class ToltecPowerLoadingModelConfig(DataclassNamespace):
    ...
    _namespace_from_dict_schema = Schema(
        {
            Literal("atm_model_name", description="The atmosphere model to use."): Or(
                None, "am_q25", "am_q50", "am_q75", "toast"
            ),
            Optional(
                "tel_surface_rms",
                default=None,
                description="The telescope surface RMS.",
            ): Or(None, PhysicalTypeSchema("length")),
            Optional(
                "det_noise_factor",
                default=None,
    ...
    _namespace_to_dict_schema = Schema(
        {
            "atm_model_params": Use(lambda c: c.to_dict() if c is not None else None),
            str: object,
        }
    )
